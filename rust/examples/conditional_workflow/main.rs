...
impl ConditionalWorkflow {
    fn new(engine: WorkflowEngine, llm_client: Arc<MockLlmClient>, input_content: String) -> Self {
        ...
        Self {
            state: WorkflowState::new(Some("ConditionalWorkflow".to_string()), Some(metadata)),
            engine,
            llm_client,
            input_content,
            category: None,
    ...
    fn create_technical_analysis_task(&self) -> task::WorkflowTask<String> {
        ...
        task::task("technical_analysis", move || async move {
            ...
            let user_prompt = format!(
                "Analyze the following technical content in detail:\n\n{}",
                input_content
            );
    ...
    fn create_business_summary_task(&self) -> task::WorkflowTask<String> {
        ...
        task::task("business_summary", move || async move {
            ...
            let user_prompt = format!(
                "Create an executive summary of the following business content:\n\n{}",
                input_content
            );
    ...
    fn create_creative_expansion_task(&self) -> task::WorkflowTask<String> {
        ...
        task::task("creative_expansion", move || async move {
            ...
            let user_prompt = format!(
                "Expand creatively on the following content:\n\n{}",
                input_content
            );
    ...
    fn create_factual_response_task(&self) -> task::WorkflowTask<String> {
        ...
        task::task("factual_response", move || async move {
            ...
            let system_prompt =
                "You are a helpful assistant. Provide accurate, factual answers to questions.";
            let user_prompt = format!(
                "Please answer the following question with factual information:\n\n{}",
                input_content
            );
    ...
    fn create_metadata_enrichment_task(
        &self,
        content: String,
    ) -> task::WorkflowTask<HashMap<String, String>> {
        let llm_client = self.llm_client.clone();
...
impl Workflow for ConditionalWorkflow {
    async fn run(&mut self) -> Result<WorkflowResult> {
        ...
        self.state
            .set_metadata("content_category", format!("{:?}", category));
        ...
        self.state
            .set_metadata("processing_path", format!("{:?}", processing_path));

        info!(
            "Content classified as {:?}, using processing path {:?}",
            category, processing_path
        );
        ...
        info!(
            "Step 3: Processing content using appropriate task for {:?}",
            category
        );
        ...
        if matches!(
            category,
            ContentCategory::Technical | ContentCategory::Business
        ) {
...
impl LlmClient for MockLlmClient {
    async fn complete(&self, request: CompletionRequest) -> Result<Completion> {
        ...
        let response = if prompt.contains("Classify") {
            ...
            "Here is a factual answer to your query based on reliable information sources."
                .to_string()
...
async fn main() -> Result<()> {
    ...
    for (name, content) in examples {
        ...
        let workflow =
            ConditionalWorkflow::new(engine.clone(), client.clone(), content.to_string());
