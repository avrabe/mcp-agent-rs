...
use crate::terminal::workflow::WorkflowEngine;

use super::models::{SprottyStatus, convert_to_sprotty_model};
use super::sprotty_adapter::{SprottyAction, process_sprotty_action};
use super::{Graph, GraphDataProvider, GraphEdge, GraphNode, GraphManager, GraphUpdate, GraphUpdateType};
            ...
            while let Ok(event) = workflow_events.recv().await {
                match event {
                    Message::WorkflowCreated(wf) => {
                        ...
                        let graph = create_workflow_graph(&graph_id, &wf.id, &wf.name, HashMap::new(), vec![]);
                    ...
                    Message::WorkflowTaskAdded(task) => {
                        if let Some(workflow_id) = &task.workflow_id {
                            ...
                            if let Some(state) = states_write.get_mut(workflow_id) {
                                ...
                                properties.insert("task_type".to_string(), serde_json::json!(task.task_type.clone()));
                                if let Some(desc) = &task.description {
                                    properties.insert("description".to_string(), serde_json::json!(desc.clone()));
                                }
                    ...
                    Message::WorkflowTaskStateChanged(task) => {
                        if let Some(workflow_id) = &task.workflow_id {
                            if let Some(status) = &task.status {
                                debug!("Task status changed in workflow {}: {} -> {}", 
                                ...
                                if let Some(state) = states_write.get_mut(workflow_id) {
                                    if let Some(node) = state.nodes.get_mut(&task.id) {
                                        ...
                                        if let Err(e) = manager.notify_update(update).await {
                                            error!("Failed to update node in workflow graph: {}", e);
                                        }
                                    }
                                }
                            }
                ...
                if let Some(state) = states.get_mut(workflow_id) {
                    if let Some(layout) = &mut state.layout {
                        ...
                        state.layout = Some(WorkflowLayout {
                            algorithm: request.algorithm.clone().unwrap_or_else(|| "layered".to_string()),
                            direction: request.direction.clone().unwrap_or_else(|| "DOWN".to_string()),
                            node_positions: HashMap::new(),
                        });
                    }
...
fn create_workflow_graph(
    ...
    let mut properties = HashMap::new();
    properties.insert("workflow_id".to_string(), serde_json::json!(workflow_id.to_string()));
    properties.insert("layout", serde_json::json!({
        "algorithm": "layered",
        "direction": "DOWN"
    }));
...
impl GraphDataProvider for WorkflowGraphProvider {
    ...
    fn get_graph_ids(&self) -> Vec<String> {
        let states = self.workflow_states.try_read();
    ...
    fn get_graph(&self, graph_id: &str) -> Option<Graph> {
        if !graph_id.starts_with("workflow-") {
            return None;
        }

        let workflow_id = graph_id.strip_prefix("workflow-")?;
        let states = self.workflow_states.try_read().ok()?;
        let state = states.get(workflow_id)?;

        let edges = state.edges.values().cloned().collect();
        Some(create_workflow_graph(
            graph_id,
            workflow_id,
            &state.name,
            state.nodes.clone(),
