...
async fn test_concurrent_writes() -> Result<(), Error> {
    ...
    println!("Concurrent write throughput: {:.2} messages/sec", throughput);
    println!("Total time: {:.2?} for {} messages from {} tasks", 
...
async fn test_write_read_cycle() -> Result<(), Error> {
    ...
    println!("Average processing time: {:.2?} per message", 
