//! # Task Execution Engine for MCP
//!
//! This module provides a robust task execution framework for the Model Context Protocol (MCP),
//! enabling asynchronous task processing, workflow signaling, and task lifecycle management.
//!
//! ## Key Components
//!
...
//! ## Features
//!
//! The executor system provides these capabilities:
//!
//! - **Concurrent task execution**: Run multiple tasks simultaneously with configurable limits
//! - **Timeouts**: Automatic task cancellation after specified durations
//! - **Retry mechanisms**: Exponential backoff with jitter for failed tasks
//! - **Result handling**: Structured success/failure handling with typed results
//! - **Workflow signaling**: Coordination between workflows using named signals
//! - **Task streaming**: Support for streaming results during long-running tasks
//! - **Metrics and telemetry**: Comprehensive performance monitoring
//!
//! ## Example Usage
//!
//! ```rust,no_run
//! use mcp_agent::mcp::executor::{AsyncioExecutor, Executor, ExecutorConfig, TaskResult};
...
//!         ..Default::default()
//!     };
//!     let executor = AsyncioExecutor::new(config);
//!
//!     // Execute a task
...
impl Executor for AsyncioExecutor {
    ...
    async fn execute(
        ...
        let timeout =
            timeout.unwrap_or_else(|| Duration::from_secs(self.config.default_timeout_secs));

        debug!(
            "Executing task {} - function: {}, timeout: {:?}",
        ...
        let handle = tokio::spawn(async move {
            ...
            while attempt < max_attempts {
                ...
                debug!(
                    "Task {} - attempt {}/{}",
                    task_id_clone, attempt, max_attempts
                );
                ...
                match time::timeout(timeout, result).await {
                    Ok(Ok(result)) => {
                        ...
                        metrics.insert(
                            "task_attempt_duration_ms",
                            attempt_duration.as_millis() as f64,
                        );
                    ...
                    Ok(Err(e)) => {
                        ...
                        warn!(
                            ...
                            task_id_clone, attempt, e
                        );

                        retry_metrics.insert(
                            "task_errors",
                            retry_metrics.get("task_errors").unwrap_or(&0.0) + 1.0,
                        );
                    ...
                    Err(_) => {
                        ...
                        retry_metrics.insert(
                            "task_timeouts",
                            retry_metrics.get("task_timeouts").unwrap_or(&0.0) + 1.0,
                        );
        ...
        match rx.await {
            ...
            Err(_) => {
                ...
                Err(McpError::Execution(
                    "Task cancelled or panicked".to_string(),
                ))
            }
        }
    }
    ...
    async fn execute_stream(
        ...
        let handle = tokio::spawn(async move {
            ...
            if function == "test_stream_success" {
                ...
                let _ = tx
                    .send(TaskResult::Failure("Stream task failed".to_string()))
                    .await;
            } else {
                // Unknown function
                let _ = tx
                    .send(TaskResult::Failure(format!(
                        "Unknown function: {}",
                        function
                    )))
                    .await;
...
mod tests {
    ...
    async fn test_execute_success() {
        ...
        let result = executor
            .execute(None, "test_success", args.clone(), None)
            .await
            .unwrap();
    ...
    async fn test_execute_timeout() {
        ...
        let result = executor
            .execute(None, "test_timeout", args, Some(Duration::from_millis(100)))
            .await;
    ...
    async fn test_execute_stream() {
        ...
        let mut rx = executor
            .execute_stream(None, "test_stream_success", args, None)
            .await
            .unwrap();
