...
impl OrchestratorWorkflow {
    fn new(engine: WorkflowEngine, llm_client: Arc<MockLlmClient>, input_data: String) -> Self {
        ...
        Self {
            state: WorkflowState::new(
                Some("OrchestratorWorkflow".to_string()),
                Some(metadata)
            ),
            engine,
            llm_client,
    ...
    fn create_data_preparation_task(&self) -> task::WorkflowTask<String> {
        ...
        task::task("data_preparation", move || async move {
            ...
            match llm_client.complete(request).await {
                Ok(completion) => {
                    let prepared_data = format!("Structured data:\n{}\n\nOriginal raw data:\n{}", 
        ...
        if let Some(sub_workflow) = self.sub_workflows.get_mut(&analysis_type) {
            ...
            match self.engine.execute_task(task).await {
                ...
                Err(e) => {
                    ...
                    sub_workflow.state.set_status("failed");
                    sub_workflow.state.set_error(format!("Analysis failed: {}", e));
                }
            }
        }
        ...
        if let Some(sub_workflow) = self.sub_workflows.get_mut(&summarization_type) {
            ...
            match self.engine.execute_task(task).await {
                ...
                Err(e) => {
                    error!("Summarization failed: {}", e);
                    sub_workflow.state.set_status("failed");
                    sub_workflow.state.set_error(format!("Summarization failed: {}", e));
                }
            }
        }
...
async fn main() -> Result<()> {
    ...
    // Sample input data
    let input_data = "This is a sample document that needs to be processed through an orchestrated workflow. 
The workflow will prepare the data, analyze it, and create a summary of the findings. 
This demonstrates how to use sub-workflows in a coordinated manner.".to_string();
