...
use std::fmt;
use mcp_agent::telemetry::{TelemetryConfig, init_telemetry, shutdown_telemetry, span_duration, add_metrics, set_error_on_current_span};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("  docker run -d --name jaeger -p 6831:6831/udp -p 16686:16686 jaegertracing/all-in-one:latest");
