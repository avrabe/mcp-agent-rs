...
use super::{Transport, TransportFactory, TransportConfig};
...
impl Transport for HttpTransport {
    async fn send_message(&self, message: Message) -> Result<(), Error> {
        ...
        if !status.is_success() {
            ...
            return Err(Error::Internal(format!("HTTP request failed with status {}: {}", status, error_text)));
        }
    ...
    async fn send_request(&self, request: Request) -> Result<Response, Error> {
        ...
        // Parse response
        let response_message: Message = response.json().await
            .map_err(|e| Error::Internal(format!("Failed to parse response JSON: {}", e)))?;
        
        // Convert Message to Response
        let mcp_response = Response::from_message(&response_message)
            .map_err(|e| Error::Internal(format!("Failed to convert message to response: {}", e)))?;
...
// Provide an empty implementation when the feature is not enabled
...
impl TransportFactory for HttpTransportFactory {
    fn create(&self) -> Result<Box<dyn Transport>, Error> {
        Err(Error::Internal("HTTP transport is not enabled. Enable the 'transport-http' feature.".to_string()))
    }
} 