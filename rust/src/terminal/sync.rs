...
impl TerminalSynchronizer {
    ...
    pub async fn register_terminal<T>(&self, terminal: Arc<T>) -> Result<(), Error>
    where
        T: Terminal + 'static,
    {
        ...
        let terminal_dyn = terminal as Arc<dyn Terminal>;
        let mut terminals = self.terminals.lock().await;
        terminals.insert(term_id, terminal_dyn);
        
        Ok(())
    }
    
    /// Unregister a terminal from the synchronizer
    pub async fn unregister_terminal(&self, terminal_id: &str) -> Result<(), Error> {
    ...
    pub async fn broadcast_output(&self, output: &str) -> Result<(), Error> {
    ...
    pub async fn broadcast_input(&self, input: &str, source_id: &str) -> Result<(), Error> {
        debug!("Broadcasting input from {}", source_id);
        
        // Send input to the input queue
        let sender = self.input_sender.lock().await;
        sender.send(input.to_string()).await.map_err(|e| {
            Error::TerminalError(format!("Failed to send input to queue: {}", e))
        })?;
    ...
    pub async fn get_next_input(&self) -> Result<String, Error> {
        debug!("Waiting for next input from any terminal");
        
        let mut receiver = self.input_queue.lock().await;
        receiver.recv().await.ok_or_else(|| {
            Error::TerminalError("Input queue closed".to_string())
        })
    }
    
    /// Send a command to a specific terminal
    pub async fn send_command(&self, terminal_id: &str, command: &str) -> Result<String, Error> {
        ...
        if let Some(terminal) = terminals.get(terminal_id) {
            ...
            Err(Error::TerminalError(format!("Terminal not found: {}", terminal_id)))
        }
    }
}
...
mod tests {
    ...
    struct MockTerminal {
        id: String,
        display_handler: Mutex<Box<dyn Fn(String) -> Result<(), Error> + Send + Sync>>,
        echo_handler: Mutex<Box<dyn Fn(String) -> Result<(), Error> + Send + Sync>>,
        command_handler: Mutex<Box<dyn Fn(String, oneshot::Sender<String>) -> Result<(), Error> + Send + Sync>>,
    }
    ...
    impl MockTerminal {
        ...
        where
            F: Fn(String, oneshot::Sender<String>) -> Result<(), Error> + Send + Sync + 'static,
        {
    ...
    async fn test_broadcast_output() {
        ...
        let mock = Arc::new(MockTerminal::new("test").with_display_handler(move |output| {
