...
static TEST_VALUE: &'static str = "test_value";
static CPU_USAGE: &'static str = "cpu_usage";
static MEMORY_USAGE: &'static str = "memory_usage";
...
fn test_alert_suppression() {
    ...
    println!("Active alerts after first trigger: {:?}", alerting.get_active_alerts());
    
    // There should be 1 alert active
    assert!(alerting.get_active_alerts().len() > 0);
    ...
    assert_eq!(active_alerts.len(), 0, "Alerts did not expire after cooldown");
    ...
    assert_eq!(active_alerts.len(), 0, "Alert triggered despite being suppressed");
    ...
    assert!(active_alerts.len() > 0, "Alert not triggered after reset");
    ...
    assert_eq!(active_alerts.len(), 0, "Alerts did not expire after second cooldown");
    ...
    alerting.process_metrics(&metrics);
    let active_alerts = alerting.get_active_alerts();
    println!("Active alerts during temporary suppression: {:?}", active_alerts);
    assert_eq!(active_alerts.len(), 0, "Alert triggered despite temporary suppression");
    ...
    alerting.process_metrics(&metrics);
    let active_alerts = alerting.get_active_alerts();
    println!("Active alerts after temp suppression expired: {:?}", active_alerts);
    assert!(active_alerts.len() > 0, "Alert not triggered after temporary suppression expired");
    assert_eq!(active_alerts[0].definition.id, "test_suppress");
}
...
fn test_alert_acknowledgement() {
    ...
    assert!(!active_alerts.is_empty(), "Alert disappeared after acknowledgement");
    assert!(active_alerts[0].acknowledged, "Alert was not marked as acknowledged");
    ...
    assert!(history[0].acknowledged, "Acknowledgement not recorded in history");
    ...
    let active_alerts = alerting.get_active_alerts();
    println!("Active alerts after cooldown: {:?}", active_alerts);
    assert_eq!(active_alerts.len(), 0, "Alert did not expire after cooldown");
    ...
    assert_eq!(active_alerts[0].definition.id, "test_ack");
    assert!(!active_alerts[0].acknowledged, "New alert should not be acknowledged");
}
...
fn test_multiple_alert_definitions() {
    ...
    assert_eq!(active_alerts.len(), 1, "Expected only CPU warning to trigger");
    ...
    let active_alerts = alerting.get_active_alerts();
    println!("Active alerts with CPU critical and memory warning: {:?}", active_alerts);
    assert_eq!(active_alerts.len(), 3, "Expected CPU warning, CPU critical, and memory warning to trigger");
    
    // Verify each type of alert is present
    let has_cpu_warning = active_alerts.iter().any(|a| a.definition.id == "cpu_warning");
    let has_cpu_critical = active_alerts.iter().any(|a| a.definition.id == "cpu_critical");
    let has_memory_warning = active_alerts.iter().any(|a| a.definition.id == "memory_warning");
    ...
    assert!(has_memory_warning, "Memory warning alert should be triggered");
}
...
fn test_alert_operator_comparisons() {
    ...
    let definitions = [equal_def, greater_than_def, less_than_def, greater_equal_def, less_equal_def];
    ...
    assert_eq!(active_alerts.len(), 3, "Expected Equal, GTE, and LTE to trigger");
    
    let has_equal = active_alerts.iter().any(|a| a.definition.id == "equal");
    let has_greater_equal = active_alerts.iter().any(|a| a.definition.id == "greater_equal");
    let has_less_equal = active_alerts.iter().any(|a| a.definition.id == "less_equal");
    
    assert!(has_equal, "Equal alert should be triggered");
    assert!(has_greater_equal, "GreaterThanOrEqual alert should be triggered");
    ...
    let active_alerts = alerting.get_active_alerts();
    println!("Active alerts for greater value (51.0): {:?}", active_alerts);
    assert_eq!(active_alerts.len(), 2, "Expected GT and GTE to trigger");
    
    let has_greater_than = active_alerts.iter().any(|a| a.definition.id == "greater_than");
    let has_greater_equal = active_alerts.iter().any(|a| a.definition.id == "greater_equal");
    ...
    let active_alerts = alerting.get_active_alerts();
    println!("Active alerts for equal value (50.0) - second check: {:?}", active_alerts);
    assert_eq!(active_alerts.len(), 3, "Expected Equal, GTE, and LTE to trigger");
    ...
    assert!(has_greater_equal, "GreaterThanOrEqual alert should be triggered");
    assert!(has_less_equal, "LessThanOrEqual alert should be triggered");
    assert!(!has_less_than, "LessThan alert should NOT be triggered for exact match");
} 