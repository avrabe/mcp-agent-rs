...
async fn test_stream_type_implementation() -> McpResult<()> {
    ...
    let tcp_stream = tokio::net::TcpStream::connect("127.0.0.1:80")
        .await
        .map_err(|_| McpError::ConnectionFailed("Test TCP stream failed".to_string()))?;
...
async fn test_message_serialization() -> McpResult<()> {
    ...
    assert_eq!(
        ...
        test_message_clone.message_type
    );
