...
mod terminal_web_code {
    ...
    use mcp_agent::terminal::graph::{Graph, GraphNode, GraphEdge, GraphUpdateType};
    ...
    pub async fn run_visualization() -> Result<()> {
        ...
        if let Some(graph_manager) = graph_manager {
            ...
            // Register the graph
            graph_manager.register_graph(workflow_graph.clone()).await.unwrap();
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    node.properties.insert("importance".to_string(), json!("high"));
                    
                    // Update the node
                    graph_manager_clone.update_node("workflow-graph", node).await.unwrap_or_else(|e| {
    ...
    fn create_sample_workflow_graph() -> Graph {
        ...
        // Add properties
        graph.properties.insert("description".to_string(), json!("A sample workflow graph for visualization demo"));
    ...
    fn create_sample_agent_graph() -> Graph {
        ...
        // Add properties
        graph.properties.insert("description".to_string(), json!("A sample agent graph for visualization demo"));
...
async fn main() -> Result<()> {
    ...
    #[cfg(not(feature = "terminal-web"))]
    {
        ...
        println!("Please rebuild with: cargo build --example simple_visualization --features terminal-web");
    }
